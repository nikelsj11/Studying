...
public class Maze {
	...
	public void start() {
		...
		for (i=0; i<HEIGHT; i++)
			for (j=0; j<WIDTH; j++)
			{
				switch (DirectionMap.MazeDefine[i].charAt(j))
				{
				case ' ':
					k=BLANK;
					break;
	...
	void DrawWall(Graphics g) {
		...
		for (i=0; i<HEIGHT; i++)
		{
			for (j=0; j<WIDTH; j++)
			{
				for (iDir= DirectionMap.RIGHT; iDir<= DirectionMap.DOWN; iDir++)
				{
					if (iMaze[i][j]==DOOR)
					{
					...
					case DirectionMap.UP:
						...
						if (iMaze[i-1][j]==WALL)
							break;
						DrawBoundary(g, j, i-1, DirectionMap.DOWN);
						break;
					case DirectionMap.RIGHT:
						if (j==WIDTH-1)	break;
						if (iMaze[i][j+1]==WALL)
							break;
						DrawBoundary(g, j+1,i, DirectionMap.LEFT);
						break;
					case DirectionMap.DOWN:
						if (i==HEIGHT-1)	break;
						if (iMaze[i+1][j]==WALL)
							break;
						DrawBoundary(g, j,i+1, DirectionMap.UP);
						break;
					case DirectionMap.LEFT:
						if (j==0)	break;
						if (iMaze[i][j-1]==WALL)
							break;
						DrawBoundary(g, j-1,i, DirectionMap.RIGHT);
						break;
					default:	
					}
				}
