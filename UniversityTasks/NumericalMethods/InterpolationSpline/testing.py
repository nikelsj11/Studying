#-*- coding: utf-8 -*-
from math import sin, pi, cos, exp, log, tan, sqrt
import matplotlib.pyplot as pyplot
from InterpolationSpline import InterpolationSpline

if __name__ == "__main__":
    # func_ - для генерации значений интерполяций, func_n - название функции (для вывода)
    func_ = lambda t: exp(cos(t)-sin(t))
    func_n = 'F(t) = exp(cos(t)-sin(t)); F'

    # генерируем начальные значения для построения полинома
    start_x = [x * (pi/4) for x in range(0, 16)]
    start_y = [func_(i) for i in start_x]

    # инициализируем класс отвечающий за интерполяцию
    interpolate = InterpolationSpline(start_x, start_y)
    ...
    print 'Значение %s(π/6)' % func_n, func_(pi)
    ...
    in_x = [round(x * 0.5, 1) for x in range(-6, end_point)]
